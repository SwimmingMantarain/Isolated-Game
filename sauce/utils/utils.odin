...
vector_from_direction :: proc(dir: Direction) -> Vec2 {
	return Vec2 {
		f32(cardinal_direction_offset(int(dir)).x),
		f32(cardinal_direction_offset(int(dir)).y),
	}
}
...
fire :: proc(cmd: ..string) -> os2.Error {
	process, start_err := os2.process_start(
		os2.Process_Desc{command = cmd, stdout = os2.stdout, stderr = os2.stderr},
	)
	if start_err != nil {
