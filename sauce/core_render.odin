...
core_render_frame_end :: proc() {
	...
	// Use views instead of images (August 2025 API change)
...

	{
		...
		sg.apply_uniforms(
			UB_Shader_Data,
			{ptr = &draw_frame.shader_data, size = size_of(Shader_Data)},
		)
...
load_sprites_into_atlas :: proc() {
	...
	// pack sprites into atlas
	{
		...
		for img, id in sprites {
			...
			append(
				&rects,
				stbrp.Rect{id = auto_cast id, w = Coord(img.width + 2), h = Coord(img.height + 2)},
			)
		}
		...
		// setup image for GPU (August 2025 API - mip_levels structure + views)
		desc: sg.Image_Desc
		...
		// Create a texture view from the image (August 2025 resource views API)
		atlas.sg_view = sg.make_view({texture = {image = atlas.sg_image}})
	}
}
...
Atlas :: struct {
	w, h:     int,
	sg_image: sg.Image,
	sg_view:  sg.View, // New in August 2025 resource views update
}
atlas: Atlas
...
load_font :: proc() {
	...
	// Create a texture view from the image (August 2025 resource views API)
	font.sg_view = sg.make_view({texture = {image = font.sg_image}})
}
